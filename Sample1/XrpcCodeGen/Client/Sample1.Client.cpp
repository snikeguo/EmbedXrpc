...
auto result=RpcObject->Send(RpcObject,sm.Index+4,RpcObject->Buffer);
sm.Reset();
if(result==false)
{
...
auto waitstate=RpcObject->Wait(Inter_Add_ServiceId,&Inter_Add_RequestResponseContent_Type,&reqresp);
...
auto waitstate=RpcObject->Wait(Inter_NoArg_ServiceId,&Inter_NoArg_RequestResponseContent_Type,&reqresp);
reqresp.State=waitstate;
exi:
RpcObject->porter->ReleaseMutex(RpcObject->ObjectMutexHandle);
return reqresp;
