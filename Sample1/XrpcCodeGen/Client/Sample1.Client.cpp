...
DateTimeChange_Return& DateTimeChange_Requester::NoOs_QueryServiceState(RequestParameter* rp)
{
	SerializationManager sm;
	El_Memset(&sm, 0, sizeof(SerializationManager));
	...
	{
		DateTimeChange_reqresp.State = RequestResponseState::ResponseState_Timeout;
		return DateTimeChange_reqresp;
	}
	...
	else if (RpcObject->CurrentReceivedData.Sid == DateTimeChange_ServiceId)
	{
		sm.Index = 0;
		sm.BufferLen = RpcObject->CurrentReceivedData.DataLen;
		sm.Buf = RpcObject->CurrentReceivedData.Data;
		DateTimeChange_Return_Deserialize(&sm, &DateTimeChange_reqresp, rp->IsIsr);
...
Test2_Return& Test2_Requester::NoOs_QueryServiceState(RequestParameter* rp)
{
	SerializationManager sm;
	El_Memset(&sm, 0, sizeof(SerializationManager));
	uint32_t nowTick = El_GetTick(rp->IsIsr);
	if ((RequestTick + RpcObject->TimeOut) < nowTick)
	{
		Test2_reqresp.State = RequestResponseState::ResponseState_Timeout;
		return Test2_reqresp;
	}
	if (RpcObject->CurrentReceivedData.Sid == EmbedXrpcNoReceivedSid)
	{
	...
	else if (RpcObject->CurrentReceivedData.Sid == Test2_ServiceId)
	{
		sm.Index = 0;
		sm.BufferLen = RpcObject->CurrentReceivedData.DataLen;
		sm.Buf = RpcObject->CurrentReceivedData.Data;
	...
	{
		Test2_reqresp.State = RequestResponseState::ResponseState_SidError;
		return Test2_reqresp;
	}
}
...
NoReturn_Return& NoReturn_Requester::NoOs_QueryServiceState(RequestParameter* rp)
{
	SerializationManager sm;
	El_Memset(&sm, 0, sizeof(SerializationManager));
	uint32_t nowTick = El_GetTick(rp->IsIsr);
	if ((RequestTick + RpcObject->TimeOut) < nowTick)
	{
		NoReturn_reqresp.State = RequestResponseState::ResponseState_Timeout;
		return NoReturn_reqresp;
	}
	if (RpcObject->CurrentReceivedData.Sid == EmbedXrpcNoReceivedSid)
	{
	...
	else if (RpcObject->CurrentReceivedData.Sid == NoReturn_ServiceId)
	{
		sm.Index = 0;
		sm.BufferLen = RpcObject->CurrentReceivedData.DataLen;
		sm.Buf = RpcObject->CurrentReceivedData.Data;
		NoReturn_Return_Deserialize(&sm, &NoReturn_reqresp, rp->IsIsr);
...
NoArgAndReturn_Return& NoArgAndReturn_Requester::NoOs_QueryServiceState(RequestParameter* rp)
{
	SerializationManager sm;
	El_Memset(&sm, 0, sizeof(SerializationManager));
	uint32_t nowTick = El_GetTick(rp->IsIsr);
	if ((RequestTick + RpcObject->TimeOut) < nowTick)
	{
		NoArgAndReturn_reqresp.State = RequestResponseState::ResponseState_Timeout;
		return NoArgAndReturn_reqresp;
	}
	if (RpcObject->CurrentReceivedData.Sid == EmbedXrpcNoReceivedSid)
	{
	...
	else if (RpcObject->CurrentReceivedData.Sid == NoArgAndReturn_ServiceId)
	{
		sm.Index = 0;
		sm.BufferLen = RpcObject->CurrentReceivedData.DataLen;
		sm.Buf = RpcObject->CurrentReceivedData.Data;
	...
	{
		NoArgAndReturn_reqresp.State = RequestResponseState::ResponseState_SidError;
		return NoArgAndReturn_reqresp;
	}
}
