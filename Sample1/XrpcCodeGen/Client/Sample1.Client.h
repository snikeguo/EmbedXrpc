...
void Invoke(EmbedXrpcConfig<DTL> *rpcConfig,DTL* userDataOfTransportLayer,SerializationManager<DTL> *recManager)
{
...
DateTimeChange_Parameter_FreeData<DTL>(&request);
}
};
...
TestDelegate_Parameter_FreeData<DTL>(&request);
}
};
...
{
Inter_Add_Return_FreeData<DTL>(response);
}
}
...
if(response->State==ResponseState_Ok)
{
Inter_NoArg_Return_FreeData<DTL>(response);
}
}
