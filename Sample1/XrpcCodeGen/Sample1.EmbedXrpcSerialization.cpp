...
{
  if (sm->Buf)
    *(bool *)(&sm->Buf[sm->Index]) = obj->boolTest;
  sm->Index += 1;
  ...
  if (sm->Buf)
    *((uint32_t *)(&sm->Buf[sm->Index])) = bitsTempValue0;
  sm->Index += 4;
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->dataLen;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  ...
  if (sm->Buf)
    *((uint64_t *)(&sm->Buf[sm->Index])) = bitsTempValue1;
  sm->Index += 8;
  if (sm->Buf)
    *(uint32_t *)(&sm->Buf[sm->Index]) = obj->Sum7;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
...
void Student_Serialize(SerializationManager *sm, Student *obj)
{
#if MyMacro == TRUE
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->a;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
#endif // #if MyMacro==TRUE
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->b;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->AddressType;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  ...
  case Student_u1_FieldNumber:
    if (sm->Buf)
      *(uint8_t *)(&sm->Buf[sm->Index]) = obj->u1;
    sm->Index += 1;
    El_Assert(sm->Index <= sm->BufferLen);
    break;
#endif // #if MyMacro==TRUE
  ...
  case Student_u2_FieldNumber:
    if (sm->Buf)
      *(uint64_t *)(&sm->Buf[sm->Index]) = obj->u2;
    sm->Index += 8;
    El_Assert(sm->Index <= sm->BufferLen);
    break;
#endif // #if MyMacro==TRUE
...
#if MyMacro == TRUE
  if (sm->Buf)
    *(uint8_t *)(&sm->Buf[sm->Index]) = obj->uend1;
  sm->Index += 1;
  El_Assert(sm->Index <= sm->BufferLen);
#endif // #if MyMacro==TRUE
#if MyMacro == TRUE
  if (sm->Buf)
    *(uint8_t *)(&sm->Buf[sm->Index]) = obj->uend2;
  sm->Index += 1;
  El_Assert(sm->Index <= sm->BufferLen);
#endif // #if MyMacro==TRUE
}
...
{
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Year;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Month;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Day;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Hour;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Min;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->Sec;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(Sex *)(&sm->Buf[sm->Index]) = obj->Sex;
  sm->Index += 8;
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(uint8_t *)(&sm->Buf[sm->Index]) = obj->DateStringLen;
  sm->Index += 1;
  El_Assert(sm->Index <= sm->BufferLen);
#if MyMacro == TRUE
  ...
  {
    if (sm->Buf)
      *(uint8_t *)(&sm->Buf[sm->Index]) = obj->DateString[DateString_index];
    sm->Index += 1;
    El_Assert(sm->Index <= sm->BufferLen);
  }
#endif // #if MyMacro==TRUE
...
{
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->EnumArrayLen;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
  for (int32_t EnumArray_index = 0; EnumArray_index < obj->EnumArrayLen; EnumArray_index++)
  {
    if (sm->Buf)
      *(Sex *)(&sm->Buf[sm->Index]) = obj->EnumArray[EnumArray_index];
    sm->Index += 8;
    El_Assert(sm->Index <= sm->BufferLen);
  }
  if (sm->Buf)
    *(int32_t *)(&sm->Buf[sm->Index]) = obj->ObjectArrayLen;
  sm->Index += 4;
  El_Assert(sm->Index <= sm->BufferLen);
...
{
  if (sm->Buf)
    *(RequestResponseState *)(&sm->Buf[sm->Index]) = obj->State;
  sm->Index += 1;
  El_Assert(sm->Index <= sm->BufferLen);
}
...
void Add_Parameter_Serialize(SerializationManager *sm, Add_Parameter *obj)
{
  ...
  {
    if (sm->Buf)
      *(uint8_t *)(&sm->Buf[sm->Index]) = obj->data[data_index];
    sm->Index += 1;
    El_Assert(sm->Index <= sm->BufferLen);
  }
...
void NoArg_Return_Serialize(SerializationManager *sm, NoArg_Return *obj)
{
  ...
  El_Assert(sm->Index <= sm->BufferLen);
  if (sm->Buf)
    *(bool *)(&sm->Buf[sm->Index]) = obj->ReturnValue;
  sm->Index += 1;
  El_Assert(sm->Index <= sm->BufferLen);
}
